...
class AppState:
    def __init__(self, *args, **kwargs):
        ...
        self.WIN_NAME = "RealSense Pointcloud"
...
def mouse_cb(event, x, y, flags, param):
     global state, depth_frame, depth_intrinsics
     if event == cv2.EVENT_LBUTTONDOWN:
        # Convert window coordinates to viewport coordinates
        viewport_x = x * state.viewport_width / state.window_width
        viewport_y = y * state.viewport_height / state.window_height
        
        # Convert viewport coordinates to point cloud coordinates
        pc_x = int(viewport_x * w / state.viewport_width)
        pc_y = int(viewport_y * h / state.viewport_height)
        
        state.clicked_x, state.clicked_y = pc_x, pc_y
        print(f"Clicked at point cloud coordinates: ({pc_x}, {pc_y})")
        if state.clicked_x is not None and state.clicked_y is not None:
            if 0 <= state.clicked_x < w and 0 <= state.clicked_y < h:
                depth_value = np.asanyarray(depth_frame.get_data())[state.clicked_y, state.clicked_x]
                depth_in_meters = depth_value * depth_scale  # Convert depth to meters
                # Convert 2D click to 3D
                depth_point = rs.rs2_deproject_pixel_to_point(depth_intrinsics, [state.clicked_x, state.clicked_y], depth_in_meters)
                print(f"3D coordinates: {depth_point}")
...
print(f"Initial window size: {state.window_width}x{state.window_height}")
...
def on_mouse_press(x, y, button, modifiers):
    handle_mouse_btns(x, y, button, modifiers)
    mouse_cb(cv2.EVENT_LBUTTONDOWN, x, window.height - y, 0, None)
...
def on_resize(width, height):
    ...
    # Maintain aspect ratio of the point cloud
    aspect_ratio = w / h
    if width / height > aspect_ratio:
        state.viewport_height = height
        state.viewport_width = int(height * aspect_ratio)
    else:
        state.viewport_width = width
        state.viewport_height = int(width / aspect_ratio)
    
    # Center the viewport
    x = (width - state.viewport_width) // 2
    y = (height - state.viewport_height) // 2
    gl.glViewport(x, y, state.viewport_width, state.viewport_height)
...
def on_draw():
    ...
    # Set up the viewport
    gl.glViewport(
        (state.window_width - state.viewport_width) // 2,
        (state.window_height - state.viewport_height) // 2,
    ...
    gl.glLoadIdentity()
    gl.gluPerspective(60, state.viewport_width / float(state.viewport_height), 0.01, 20)
    ...
    gl.glMatrixMode(gl.GL_PROJECTION)
    gl.glLoadIdentity()
    gl.glOrtho(0, state.viewport_width, 0, state.viewport_height, -1, 1)
    gl.glMatrixMode(gl.GL_MODELVIEW)
    gl.glLoadIdentity()
    gl.glMatrixMode(gl.GL_TEXTURE)
    gl.glLoadIdentity()
...
def run(dt):
    global w, h, depth_frame, depth_intrinsics
    # window.set_caption("RealSense (%dx%d) %dFPS (%.2fms) %s" %
    #                    (w, h, 0 if dt == 0 else 1.0 / dt, dt * 1000,
    #                     "PAUSED" if state.paused else ""))

    window.set_caption(f"RealSense ({w}x{h}) {0 if dt == 0 else 1.0 / dt:.0f}FPS ({dt * 1000:.2f}ms) {'PAUSED' if state.paused else ''} - Window: {state.window_width}x{state.window_height} - Viewport: {state.viewport_width}x{state.viewport_height}")
    if state.paused:
        return
